...
const createStaffSchema = Joi.object({
  ...
  staffCategory: Joi.string().valid('barber', 'hairstylist', 'nail_technician', 'massage_therapist', 'esthetician', 'receptionist', 'manager', 'trainer', 'secretary_general', 'other').optional(),
  gender: Joi.string().valid('male', 'female', 'other').optional(),
...
router.get('/staff', authenticateToken, requireAdmin, async (req: AuthRequest, res) => {
  try {
    ...
    const query: any = {
      role: { $in: ['barber', 'hairstylist', 'nail_technician', 'massage_therapist', 'esthetician', 'receptionist', 'manager', 'trainer', 'secretary_general'] }
    };
...
router.patch('/superadmin/users/:id', authenticateToken, requireSuperAdmin, async (req: AuthRequest, res) => {
  try {
    ...
    if (role && ['barber', 'hairstylist', 'nail_technician', 'massage_therapist', 'esthetician', 'receptionist', 'manager', 'trainer', 'secretary_general', 'owner'].includes(role) && !salonId && !existingUser?.salonId) {
