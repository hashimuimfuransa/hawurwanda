...
const AdminPanel: React.FC = () => {
  ...
  const createSalonMutation = useMutation({
    mutationFn: (salonData: any) => adminService.createSalon(salonData),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['admin-salons'] });
  ...
  const updateStaffMutation = useMutation({
    mutationFn: ({ staffId, staffData }: { staffId: string; staffData: FormData }) => 
      adminService.updateStaffMember(staffId, staffData),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['admin-staff'] });
  ...
  const handleDownloadReport = async () => {
    try {
      ...
      const reportData = {
        ...
        bookingStats: reports.bookingStats || [],
        revenueStats: reports.revenueStats || {},
        salonStats: reports.salonStats || {},
      };
      ...
      const csvContent = [
        ...
        `Total Salons,${reportData.summary.totalSalons}`,
        ...
        'User Distribution:',
        'Role,Count',
        ...(reportData.userDistribution.map((item: any) => `${item._id || 'Unknown'},${item.count}`) || []),
        '',
        'Staff Gender Distribution:',
        'Gender,Count',
        ...(reportData.staffGenderDistribution.map((item: any) => `${item._id || 'Not specified'},${item.count}`) || []),
        '',
        'Booking Stats:',
        'Status,Count',
        ...(reportData.bookingStats.map((item: any) => `${item._id || 'Unknown'},${item.count}`) || []),
      ].join('\n');
  ...
  const handleViewStaffProfile = (staff: any) => {
  ...
  const handleEditStaff = (staff: any) => {
    setSelectedStaff(staff);
    // Populate form data with staff details
    setStaffFormData({
      name: staff.name || '',
      email: staff.email || '',
      phone: staff.phone || '',
      ...
      birthYearRange: staff.birthYearRange || '',
      bio: staff.bio || '',
  ...
  return (
    <div className="relative">
      ...
      <CreateStaffModal
        showModal={showCreateStaffModal}
        onClose={() => setShowCreateStaffModal(false)}
        onSubmit={(staffData) => createStaffMutation.mutate(staffData)}
        salons={salons}
        isPending={createStaffMutation.isPending}
      />

      {/* Create Salon Modal - Moved to bottom of file */}
      ...
      {/* Edit User Modal - Component not available */}
            ...
            <div className="p-6 space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-900">{selectedStaff.gender || 'Not specified'}</p>
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">Category</label>
                  <p className="text-sm text-gray-900">{selectedStaff.staffCategory || 'Not specified'}</p>
                </div>
                {selectedStaff.nationalId && (
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1">National ID</label>
                    <p className="text-sm text-gray-900">{selectedStaff.nationalId}</p>
                  </div>
                )}
                {selectedStaff.educationLevel && (
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1">Education Level</label>
                    <p className="text-sm text-gray-900">{selectedStaff.educationLevel}</p>
                  </div>
                )}
                {selectedStaff.birthYearRange && (
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1">Birth Year Range</label>
                    <p className="text-sm text-gray-900">{selectedStaff.birthYearRange}</p>
                  </div>
                )}
              </div>
              ...
              {selectedStaff.credentials && selectedStaff.credentials.length > 0 && (
                <div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {selectedStaff.credentials.map((credential: string, index: number) => (
                      <span
                        key={index}
              ...
              {selectedStaff.bio && (
                <div>
                  ...
                  <p className="text-sm text-gray-900">{selectedStaff.bio}</p>
                </div>
              )}
      ...
      {/* Edit Staff Modal */}
      {showStaffEditModal && selectedStaff && (
              ...
              <div className="space-y-6">
                ...
                <div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) => {
                          ...
                          setStaffFormData({ ...staffFormData, specialties });
                        }}
                        className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                    ...
                    <div>
                      <label className="block text-sm font-medium text-gray-700 mb-2">Credentials/Certifications (comma-separated)</label>
                      <input
                        type="text"
                        value={staffFormData.credentials.join(', ')}
                        onChange={(e) => {
                          const credentials = e.target.value.split(',').map(c => c.trim()).filter(c => c);
                          setStaffFormData({ ...staffFormData, credentials });
                        }}
                        className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
              ...
              <div className="flex justify-end space-x-3 pt-4 border-t border-gray-200">
                ...
                <button
                  onClick={() => {
                    ...
                    // Add basic fields
                    updateData.append('name', staffFormData.name);
                    if (staffFormData.email) updateData.append('email', staffFormData.email);
                    updateData.append('phone', staffFormData.phone);
                    if (staffFormData.nationalId) updateData.append('nationalId', staffFormData.nationalId);
                    if (staffFormData.gender) updateData.append('gender', staffFormData.gender);
                    if (staffFormData.staffCategory) updateData.append('staffCategory', staffFormData.staffCategory);
                    if (staffFormData.experience) updateData.append('experience', staffFormData.experience);
                    if (staffFormData.educationLevel) updateData.append('educationLevel', staffFormData.educationLevel);
                    if (staffFormData.birthYearRange) updateData.append('birthYearRange', staffFormData.birthYearRange);
                    if (staffFormData.bio) updateData.append('bio', staffFormData.bio);
                    
                    // Add arrays as JSON strings
                    updateData.append('specialties', JSON.stringify(staffFormData.specialties));
                    updateData.append('credentials', JSON.stringify(staffFormData.credentials));
                    
                    // Add salon ID if selected
                    if (staffFormData.salonId) updateData.append('salonId', staffFormData.salonId);
                    
                    // Update staff member
                    updateStaffMutation.mutate({ staffId: selectedStaff._id, staffData: updateData });
                  }}
